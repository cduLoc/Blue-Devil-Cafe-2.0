...
def main():
    # What was added
    # arrays for our data. Using our names dummy ID's and meals
    #S_names = ["Nathan Andrews", "Carl Wilburn", "Yvonne Xu"]
    #S_username = ["nandrews", "cwilburn", "yxu"]
    #S_ID = ["000079", "000426", "000380"]
    #S_meals = ["10", "5", "12"]
    S_names = []
    S_username = []
    S_ID = []
    ...
    file1 = open("StudentInfo.txt", "r")
    S_names = file1.readline().split(',')
    S_username = file1.readline().split(',')
    S_ID = file1.readline().split(',')
    S_meals = file1.readline().split(',')
    print(S_username)
    file1.close()

    value = input("Please enter username: ")
    if value in S_username:
        index = S_username.index(value)
        print("\nName:", S_names[index])
        print("ID:", S_ID[index])
        print("Meals left:", S_meals[index])
    else:
        while value not in S_ID:
